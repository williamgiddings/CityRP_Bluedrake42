...
function ENT:Draw()
	...
	cam.Start3D2D(self:GetPos() + up, eyeAng, 0.05)
		surfaceSetDrawColor(0, 200, 255, 255)
		...
		for k, v in ipairs(self.attachmentNames) do
			if ply.CWAttachments[v.name] then
				drawShadowText(v.display, baseFont, 0, v.vertPos, gray, black, 2, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			else
				drawShadowText(v.display, baseFont, 0, v.vertPos, green, black, 2, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		end
	cam.End3D2D()
end