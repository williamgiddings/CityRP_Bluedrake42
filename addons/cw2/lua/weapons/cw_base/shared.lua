...
function SWEP:Deploy()
	...
	self:SetNextSecondaryFire(CT + self.DeployTime / self.DrawSpeed)
	self:SetNextPrimaryFire(CT + self.DeployTime / self.DrawSpeed)
	self.ReloadWait = CT + self.DeployTime / self.DrawSpeed
	self.HolsterWait = CT + self.DeployTime / self.DrawSpeed
...
function SWEP:CalculateSpread(vel)
	...
	if not self.Owner.LastView then
		...
		self.Owner.ViewAff = LerpCW20(0.25, self.Owner.ViewAff, (aim - self.Owner.LastView):Length() * 0.5)
		self.Owner.LastView = aim
	end
	...
	if self.dt.State == CW_AIMING then
		self.BaseCone = self.AimSpread * (aimSpreadMod or 1)
	else
		if self.dt.BipodDeployed then
			self.BaseCone = self.AimSpread * 1.2 * (aimSpreadMod or 1)
		else
			self.BaseCone = self.HipSpread * (hipSpreadMod or 1)
		end
	end
	
	if self.Owner:Crouching() then
		self.BaseCone = self.BaseCone * (self.dt.State == CW_AIMING and 0.9 or 0.75)
	end
	
	self.CurCone = self:getFinalSpread(vel, maxSpreadMod)
	
	if CT > self.SpreadWait then
		self.AddSpread = math.Clamp(self.AddSpread - 0.005 * self.AddSpreadSpeed, 0, self.MaxSpreadInc * maxSpreadMod)
