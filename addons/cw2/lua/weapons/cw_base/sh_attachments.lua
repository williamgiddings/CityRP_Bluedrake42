...
function SWEP:canAttachSpecificAttachment(attachmentName, ply, imaginaryAttachments, imaginaryActiveAttachments)
	...
	for category, data in pairs(self.Attachments) do
		for key, attachment in ipairs(data.atts) do
			if attachment == attachmentName then
				...
				if CustomizableWeaponry:hasAttachment(ply, att.name) then
