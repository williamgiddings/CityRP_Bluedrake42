...
if CLIENT then
	...
	function CustomizableWeaponry.sightAdjustment:adjust(wep, direction, adjustAmount)
		...
		local modelData = self.currentModelData
		local adjustRange = modelData.adjustment
		...
		local lastOffset = math.floor(self:getOffset(self.currentModelData, self.currentModelData.adjustment) * 20) -- floor + multiply by 20 to achieve increases in the value every 5
		
		modelData.pos[adjustRange.axis] = math.Clamp(modelData.pos[adjustRange.axis] + range * direction, adjustRange.min, adjustRange.max)
		
		local curOffset = math.floor(self:getOffset(self.currentModelData, self.currentModelData.adjustment) * 20)
	...
	function CustomizableWeaponry.sightAdjustment:setCurrentAttachment(attData, modelData)
	...
	function CustomizableWeaponry.sightAdjustment:canSetAttachment(wep, category)
		...
		if attCategory then -- if there is one, check if there is an active attachment
			...
			if attachment then
				local modelData = wep.AttachmentModelsVM[attachment]
				
				if modelData and modelData.adjustment then -- if there is an attachment like this, and it can be adjusted, set it as the current adjustable attachment
					return attCategory, modelData
				end
			end
		end
	end
	...
	function CustomizableWeaponry.sightAdjustment:attemptSetAttachment(wep, category)
		local attData, modelData = self:canSetAttachment(wep, category)
		
		if attData and modelData then
			CustomizableWeaponry.sightAdjustment:setCurrentAttachment(attData, modelData)
			return true
		end
	...
	function CustomizableWeaponry.sightAdjustment:getOffset(modelData, adjustRange)
		local range = math.abs(adjustRange.min - adjustRange.max)
		local curPos = modelData.pos[adjustRange.axis]
	...
	function CustomizableWeaponry.sightAdjustment:setDefaultOffset(wep)
		...
		local attData, modelData = self.currentAttData, self.currentModelData
		local curAttName = attData.atts[attData.last]
		
		offsets[curAttName] = modelData.pos[modelData.adjustment.axis] -- set the default offset
		...
		if file.Exists(fileName, "DATA") then -- if we currently have a file that contains default offsets, load it up so that we can append more data to it
			...
			for attName, offset in pairs(offsets) do
				...
				if att and att.adjustment then -- check for adjustment in case an adjustment for some attachment gets removed in later versions
					att.pos[att.adjustment.axis] = offset
				end
			end
		end
	end
	...
	function CustomizableWeaponry.sightAdjustment:draw(wep)
		...
		if not self.currentModelData then
			...
			for k, v in pairs(wep.Attachments) do
				local attData, modelData = self:canSetAttachment(wep, k)
				...
				if attData and modelData then
					...
					local offset = self:getOffset(modelData, modelData.adjustment)
			...
			local attData, modelData = self.currentAttData, self.currentModelData
			local adjustRange = modelData.adjustment
			local x, y = attData.offset[1], attData.offset[2]
			
			local dist = self:getOffset(modelData, adjustRange)
			
			draw.ShadowText("Adjusting category: " .. attData.header, hud36, x, y + 6, wep.HUDColors.white, wep.HUDColors.black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
