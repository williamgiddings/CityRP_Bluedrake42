...
function CustomizableWeaponry:postSpawn()
	...
	if CustomizableWeaponry.useAttachmentPossessionSystem then
		if SERVER then
...
function CustomizableWeaponry:hasAttachment(ply, att)
	if not self.useAttachmentPossessionSystem then
		return true
	end
	
	if ply.CWAttachments[att] then
		return true
	end
...
function CustomizableWeaponry:giveAttachments(tbl, dontNotify)
